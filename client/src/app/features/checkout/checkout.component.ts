...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  async confirmPayment(stepper: MatStepper) {
    ...
    try {
      ...
      if (token) {
        const result = await this.stripeService.confirmPayment(token);
        ...
        if (result.paymentIntent?.status === 'succeeded') {
          ...
          throw new Error('Quelque chose s\'est mal passé lors de la confirmation du paiement');
        }
      }
    } catch (error: any) {
  ...
  private async createOrderModel() : Promise<OrderToCreate> {
    ...
    if(!cart?.id || !cart.deliveryMethodId || !shippingAddress || !card) {
      throw new Error('Problème lors de la création de la commande');
    }
